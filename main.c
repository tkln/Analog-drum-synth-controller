...
static inline void mux_sample(uint8_t adr, uint8_t val);

static inline void mux_scan_buffer(void);
...
volatile uint8_t velocity_list[MAX_VOICES];
...
static inline void mux_sample(uint8_t adr, uint8_t val){
	DAC_VAL = val;
	//_delay_ms(10); /* this might be needed if dac takes some time to settle */
	MUX_PORT = adr&0b1111;
	MUX_PORT &= ~(1<<4);
}
...
ISR(USART_RX_vect){
	...
	switch(midi_event_stage){
		...
		case velocity:
			...
			velocity_list[midi_event_data.note] = data<<1; /* velocity is only 7 bits because midi standards */
			midi_event_stage = event_match;
		default:
			break;
	}
